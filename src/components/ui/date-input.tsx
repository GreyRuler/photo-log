...
const DateInput: React.FC<DateInputProps> = ({ value, onChange }) => {
    ...
    const validateDate = (field: keyof DateParts, value: number): boolean => {
        ...
        const d = new Date(newDate.year, newDate.month - 1, newDate.day, newDate.hour, newDate.minute)
    ...
    const handleInputChange =
        (field: keyof DateParts) => (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (isValid) {
                onChange(new Date(newDate.year, newDate.month - 1, newDate.day, newDate.hour, newDate.minute))
            }
        }
    ...
    return (
        <div className="flex border rounded-lg items-center text-sm p-1">
            <input
                type="text"
                ref={dayRef}
                max={31}
